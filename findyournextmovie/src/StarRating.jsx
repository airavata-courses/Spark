...
class StarRating extends React.Component {
  ...
  componentWillMount() {
    ...
    if(localStorage.getItem("isAuthenticated") == "true"){
        axios.get('http://149.165.168.227:8080/usermovierating/getbyuseridmovieid?user_id=' + localStorage.getItem("ACCESS_TOKEN") + '&movie_id=' + this.props.movie_id)
        .then(res => {
          this.setState({
  ...
  onStarClick(nextValue, prevValue, name) {
    if (localStorage.getItem("isAuthenticated") === null) {
      history.push("/login");
        ...
        axios.post('http://149.165.168.227:8080/usermovierating/save',  movieData )
            .then(res => {
